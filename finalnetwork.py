...
def define_netl82s2(input_ch, output_ch, init_type='kaiming', init_gain=0.02):
    # net = net1(input_c=input_ch, output_c=output_ch)
    net = net1(dim=4, input_resolution=(32, 32), depth=2, num_heads=4, window_size=8)  # input_resolution：（H,W）
    net = init_weights(net, init_type, gain=init_gain)
    # prenet1 = torch.load('./pre/l8tos2.pth')
    prenet1 = torch.load(r'./checkpoints/net1/l8tos2/model/l8tos2_latest.pth')
    net.load_state_dict(prenet1)
...
class net1(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        #     nn.Conv2d(dim, 64, 3, 1, 1, bias=True),
        #     nn.LeakyReLU())
...
def define_nets2tol8(input_ch, output_ch, init_type='kaiming', init_gain=0.02):
    # net = net2(input_c=input_ch, output_c=output_ch)
    net = net2(dim=4, input_resolution=(32, 32), depth=2, num_heads=4, window_size=8)  # input_resolution：（H,W）
    net = init_weights(net, init_type, gain=init_gain)
    prenet2 = torch.load(r'./checkpoints/net2/s2tol8/model/s2tol8_latest.pth')
    # pre = torch.load("./pre/s2tol8.pth")
    net.load_state_dict(prenet2)
    # 检查gpu是否可以用
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # net = net.to(device)
    net = net.cuda(0)
