...
# from .datapre import crop,generate_trainval_list, write_train_list
# def datanormal(x):
#     x = x.transpose(2, 0, 1)
#     channel, _, _ = x.shape
...
class Dataset(data.Dataset):
    def __init__(self, args, split, isTrain=True):
        ...
        # self.root = '../traindata'
        self.root = r'./traindata/net2_s2tol8'
        self.split = split
    ...
    def __getitem__(self, index):
        ...
        L8_90m = self.root + '/L8_90m/' + img_name
        S2_90m = self.root + '/S2_90m/' + img_name

        L8_90m = skio.imread(L8_90m).transpose(2,0,1)
        L8_90m = transform(L8_90m)

        S2_90m = skio.imread(S2_90m).transpose(2,0,1)
        S2_90m = transform(S2_90m)

        # 随机选择翻转和旋转操作 实验-8加入
        flip_num = np.random.choice(2)
        rotate_num0 = np.random.choice(2)
        rotate_num = np.random.choice(3)
        L8_90m = transform_image(L8_90m, flip_num, rotate_num0, rotate_num)
        S2_90m = transform_image(S2_90m, flip_num, rotate_num0, rotate_num)


        return {"S2_90m":S2_90m,
                'L8_90m':L8_90m,    # .copy(),
                # 'S2_real':S2_real,
                # 'L8_90m':L8_path.copy(),
                "name":img_name
